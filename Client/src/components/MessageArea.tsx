                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {Array.isArray(searchInfo.urls) ? (
                                            searchInfo.urls.map((url, index) => (
                                                <div key={index} className="bg-gray-100 text-xs px-3 py-1.5 rounded border border-gray-200 break-all max-w-xs hover:bg-gray-50 transition-all duration-200">
                                                    {typeof url === 'string' ? url : JSON.stringify(url).substring(0, 30)}
                                                </div>
                                            ))
                                        ) : (
                                            <div className="bg-gray-100 text-xs px-3 py-1.5 rounded border border-gray-200 break-all max-w-xs hover:bg-gray-50 transition-all duration-200">
                ...
                {messages.map((message) => (
                    <div key={message.id} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'} mb-5`}>
                        <div className="flex flex-col max-w-3xl">
                            ...
                            <div
                                className={`inline-block w-fit max-w-full md:max-w-2xl break-words whitespace-pre-wrap rounded-lg py-3 px-5 ${
                                    message.isUser
